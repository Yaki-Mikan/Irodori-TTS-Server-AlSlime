...
本家サーバーの Voice ファイル API は、参照音声（音声ファイル）のアップロードを対象とした設計です。AlSlime の Voice 管理には Latent（`.pt` / `.pth`）の直接登録や日本語の Voice ID を使う機能があるため、本フォークでは本家の設計を保ったまま次の拡張を加えています。
